...
func _spawn_funny(id: String) -> void:
	...
	var kind_map := {"joker": FunnyEnemy.Kind.JOKER, "rubber_duck": FunnyEnemy.Kind.RUBBER_DUCK, "pinata": FunnyEnemy.Kind.PI�ATA}
...
func kill_funny_enemy(f: FunnyEnemy) -> void:
	...
	match f.kind:
		...
		FunnyEnemy.Kind.PI�ATA:
			Audio.play("combo5", -1.0)
			shake(6.0)
			hud.show_announcer("PI�ATA SMASHED!", Color(0.0, 1.0, 0.5))
			for i in range(20):
				var p := CPUParticles2D.new()
				p.position = world.to_local(f.global_position)
